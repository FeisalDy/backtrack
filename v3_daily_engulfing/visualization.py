...
def plot_trade_distribution(res_df, symbol, output_dir=None):
    ...
    ax3 = axes[1, 0]
    direction_results = res_df.groupby('direction').apply(
        lambda x: pd.Series({
            'wins': (x['net'] > 0).sum(),
            'losses': (x['net'] <= 0).sum()
        }), include_groups=False
    )
    direction_results.plot(kind='bar', ax=ax3, color=['green', 'red'], alpha=0.7)
    ax3.set_xlabel('Direction', fontsize=11, fontweight='bold')
    ax3.set_ylabel('Count', fontsize=11, fontweight='bold')
    ax3.set_title('Wins vs Losses by Direction', fontsize=12, fontweight='bold')
