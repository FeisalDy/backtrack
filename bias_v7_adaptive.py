# ===== CRYPTO TRADING BOT - ADAPTIVE BIAS v7 =====
# 
# IMPROVEMENTS:
# 1. Adaptive thresholds based on timeframe (30m vs hour)
# 2. Market regime filter (only trade trending markets)
# 3. Stricter momentum alignment requirement
# 4. Asset-specific parameter tuning
# 5. Volume-price divergence detection
#
# TARGET: 50%+ win rate across ALL timeframes and assets
#
# QUICK START: Change symbol and interval to test different pairs
# - SYMBOL: "BTC", "ETH", "DOGE", "SOL", "ADA"
# - INTERVAL: "30m" or "hour"
#
# Available data:
#   30m data: BTC, ETH, DOGE (~60 days)
#   hour data: BTC, ETH, DOGE, SOL, ADA (~3 years)
#
# Data loaded from historical_data/ parquet files
...
# USER PARAMETERS
# -------------------------
SYMBOL = "BTC"  # BTC, ETH, DOGE (30m or hour) | SOL, ADA (hour only)
INTERVAL = "hour"  # "30m" or "hour"
LIMIT = 10000  # Max bars to use for backtest
PARQUET_DIR = "historical_data"  # Directory with parquet files
...
ASSET_PARAMS = {
    "BTC": {"score_threshold_30m": 8.0, "score_threshold_hour": 10.0},  # Increased hour threshold
    "ETH": {"score_threshold_30m": 9.0, "score_threshold_hour": 14.0},
    "DOGE": {"score_threshold_30m": 10.0, "score_threshold_hour": 15.0},
    "SOL": {"score_threshold_30m": 9.0, "score_threshold_hour": 14.0},
    "ADA": {"score_threshold_30m": 9.0, "score_threshold_hour": 14.0},
}

# -------------------------
# DATA LOADING
# -------------------------
def load_from_parquet_file(symbol, interval, parquet_dir="historical_data"):
    """
    Load historical data from pre-downloaded parquet files
    
    Args:
        symbol: Crypto symbol (BTC, ETH, DOGE, SOL, ADA)
        interval: Timeframe (30m or hour)
    ...
    if not os.path.exists(filepath):
        available = [f.replace('_USD_', ' ').replace('.parquet', '') for f in os.listdir(parquet_dir) if f.endswith('.parquet')]
        raise FileNotFoundError(
            f"File not found: {filepath}\n"
            f"Available: {', '.join(available)}"
        )
    
    print(f"Loading {symbol} {interval} data from parquet...")
    df = pd.read_parquet(filepath, engine='pyarrow')
    df['time'] = pd.to_datetime(df['time'])
    
    print(f"Loaded {len(df):,} bars | {df['time'].iloc[0].strftime('%Y-%m-%d')} to {df['time'].iloc[-1].strftime('%Y-%m-%d')} ({(df['time'].iloc[-1] - df['time'].iloc[0]).days} days)")
    
    return df

# Load data
df = load_from_parquet_file(SYMBOL, INTERVAL, PARQUET_DIR)
...
# -------------------------
# BIAS FUNCTION (ADAPTIVE v7)
# -------------------------
def is_market_trending(df, i, interval):
    """
    Market regime filter - only trade in trending markets
    Returns True if market is in a clear trend
    
    Uses different thresholds for different timeframes:
    - 30m: More lenient (faster trends)
    - hour: More strict (need clearer trends)
    """
    if i < 100:
        return False
    ...
    # Measure separation between MAs relative to volatility
    separation = abs(fast_ma - slow_ma) / (atr + 1e-8)
    
    # Adaptive threshold based on timeframe
    if interval == "30m":
        threshold = 1.0  # More lenient for 30m
    else:  # hour
        threshold = 1.8  # Stricter for hourly
    
    return separation > threshold

def get_bias(df, i, symbol, interval):
    """
    ADAPTIVE BIAS with:
    ...
    # FILTER 1: Only trade when ATR > median (volatile enough)
    if df['atr'].iloc[i] <= df['atr_median'].iloc[i]:
        return None
    
    # FILTER 2: Market regime - only for hourly timeframe
    if interval == "hour" and not is_market_trending(df, i, interval):
        return None
    ...
    # 3. Price momentum (STRICTER - all must align)
    momentum_3_z = df['price_momentum_3_zscore'].iloc[i]
    momentum_10_z = df['price_momentum_10_zscore'].iloc[i]
    
    # Check if all momentum timeframes align
    momentum_aligned_bull = (momentum_3_z > 0 and momentum_10_z > 0)
    momentum_aligned_bear = (momentum_3_z < 0 and momentum_10_z < 0)
    
    if momentum_3_z > 1.0 and momentum_10_z > 0.5:
        score += 3.0
    elif momentum_3_z < -1.0 and momentum_10_z < -0.5:
        score -= 3.0
    elif momentum_3_z > 0.5 and momentum_aligned_bull:
        score += 1.5
    elif momentum_3_z < -0.5 and momentum_aligned_bear:
        score -= 1.5
    else:
        # Penalize if momentum doesn't align
        score *= 0.7

    # 4. Volume confirmation (WITH divergence check)
    ...
    if price_up and volume_z < -0.5:  # Price up but volume down = bearish divergence
        score -= 2.0
    elif price_down and volume_z < -0.5:  # Price down but volume down = bullish divergence  
        score += 2.0
    elif volume_z > 1.5:
        if price_up:
            score += 2.5
    ...
    # ADAPTIVE THRESHOLD based on timeframe and asset
    params = ASSET_PARAMS.get(symbol, {"score_threshold_30m": 9.0, "score_threshold_hour": 12.0})
    
    if interval == "30m":
        threshold = params["score_threshold_30m"]
    else:  # hour
        threshold = params["score_threshold_hour"]
    
    # Bias determination with adaptive threshold
    if score >= threshold:
        return "bull"
...
while i < len(df)-1:
    pbar.update(1)
    bias = get_bias(df, i, SYMBOL, INTERVAL)  # Pass symbol and interval
    if bias is None:
        i += 1
        continue
...
print("\n===== BACKTEST RESULTS (ADAPTIVE BIAS v7) =====")
...
# Show adaptive parameters being used
params = ASSET_PARAMS.get(SYMBOL, {"score_threshold_30m": 9.0, "score_threshold_hour": 12.0})
threshold_used = params["score_threshold_30m"] if INTERVAL == "30m" else params["score_threshold_hour"]
print(f"Score threshold used: {threshold_used}")
...
plt.title(f'Equity Curve - {SYMBOL}/USD {INTERVAL} (Adaptive v7)')
...
res_df.to_csv(f"backtest_{SYMBOL}_{INTERVAL}_v7.csv", index=False)
print(f"\nResults saved to: backtest_{SYMBOL}_{INTERVAL}_v7.csv")
